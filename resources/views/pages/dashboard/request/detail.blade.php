                    ...
                    <form action="#" method="POST">
                        <div class="">
                            ...
                            <div class="px-1 py-4 text-right">
                                ...
                                <a href="{{ route('member.approve.request', $order->id) }}" type="button" class="inline-flex justify-center px-4 py-2 text-sm font-medium text-black bg-green-600 border border-transparent rounded-lg shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500" onclick="return confirm('Are you sure want to approve this data?')">
                                    Setujui
                                </a>
                                <a href="{{ route('member.pay.request', $order->id) }}" id="pay-button" type="button" class="inline-flex justify-center px-4 py-2 text-sm font-medium text-black bg-blue-600 border border-transparent rounded-lg shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500" onclick="return confirm('Sudah sesuai ingin lanjut membayar?')">
                                    Bayar
                                </a>
                                @else
                                <a type="button" class="inline-flex justify-center px-4 py-2 text-sm font-medium text-black bg-green-600 border border-transparent rounded-lg shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500" disabled>
                                    Setujui
                                </a>
...
<script type="text/javascript">
    ...
    if (localStorage.getItem('payButtonClicked_{{ $order->id }}')) {
        payButton.disabled = true; // Menonaktifkan tombol jika sudah diklik sebelumnya
        payButton.textContent = 'Sudah Bayar'; // Mengubah teks tombol
    }

    payButton.onclick = function(event) {
        if (localStorage.getItem('payButtonClicked_{{ $order->id }}')) {
            alert('Anda sudah melakukan pembayaran.');
            event.preventDefault(); // Menghentikan aksi default jika tombol sudah diklik sebelumnya
            return false;
        }

        localStorage.setItem('payButtonClicked_{{ $order->id }}', 'true');
        payButton.disabled = true; // Menonaktifkan tombol setelah diklik
        payButton.textContent = 'Sudah Bayar'; // Mengubah teks tombol setelah diklik
    };
</script>
